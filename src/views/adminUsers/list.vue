<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-if="checkPermission('adminUsers.list.addAdminUser')" class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-plus" @click="handleAdd">{{ $t('table.add') }}</el-button>
    </div>
    ...
    >
      ...
      <el-table-column align="center" :label="$t('table.actions')">
        <template slot-scope="scope">
          <el-button v-if="checkPermission('adminUsers.list.editAdminUser')" v-waves icon="el-icon-edit" type="primary" size="small" @click="handleEdit(scope)">{{ $t('table.edit') }}</el-button>
          <el-button v-if="checkPermission('adminUsers.list.deleteAdminUser')" v-waves icon="el-icon-delete" type="danger" :disabled="scope.row.id===1?true:false" size="small" @click="handleDelete(scope)">{{ $t('table.delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      ...
      >
        ...
        <el-form-item :label="$t('table.adminUsers.facephoto')">
          <single-image-upload :auth="auth" :image-url="users.facephoto_url" :params="{type:'facephoto'}" :width="80" :height="80" @callbackshowPath="showFacephoto" @callbackRealPath="realFacephoto" />
        </el-form-item>
...
<script>
import { getAdminUsers, deleteAdminUsers, addAdminUsers, updateAdminUsers } from '@/api/adminUsers'
...
export default {
  ...
  methods: {
    ...
    async getAdminUsers() {
      ...
      this.rolesList = res.data.roles_list
    ...
    handleEdit(scope) {
      this.users = deepClone(scope.row)

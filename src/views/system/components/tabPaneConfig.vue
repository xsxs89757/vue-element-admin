<template>
  <el-form ref="form" v-loading="loading" :model="configDetail" label-width="90px">
    <el-form-item v-for="item in list" :key="item.id" :label="item.title">
      <el-col v-if="item.type === 'string' || item.type === 'url' || item.type === 'textarea' || item.type === 'radio' || item.type === 'checkbox'" :span="24">
        <el-form-item v-if="item.type === 'string'">
          <el-input v-model="configDetail[item.name]" :placeholder="item.title" />
        </el-form-item>
      ...
      <el-col v-if="item.type === 'number' || item.type === 'password' || item.type === 'select'" :span="5">
        ...
        <el-form-item v-if="item.type === 'password'">
          <el-input v-model="configDetail[item.name]" :placeholder="item.title" show-password />
        </el-form-item>
...
import { getSystemConfig, updateConfigGroup } from '@/api/config'
import { uploadAddress } from '@/api/upload'
